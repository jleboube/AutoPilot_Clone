...
interface SidebarProps {
  activeView: string;
  setActiveView: (view: 'dashboard' | 'portfolios' | 'stocks' | 'crypto' | 'aiPicks' | 'profile') => void;
}
    ...
    <aside className="w-16 lg:w-64 bg-slate-800/50 border-r border-slate-700/50 p-4 flex flex-col justify-between transition-all duration-300">
      <div>
        ...
        <nav className="space-y-2">
          ...
          {user && (
            <NavItem icon={<UserIcon className="w-6 h-6" />} label="My Profile" isActive={activeView === 'profile'} onClick={() => setActiveView('profile')} />
          )}
